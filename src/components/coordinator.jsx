const Coordinator = ({ imageSrc, title, name, rollNo }) => {
  ...
  "
    >
      ...
      <div
        className="
          absolute top-[-35%] w-[50%] aspect-square rounded-full border-4 border-white overflow-hidden
      ...
      <div className="mt-[30%] text-center md:mt-[25px]">
        <h2 className="text-[clamp(10px,3vw,20px)] font-bold font-[lora] md:text-base">
          {title}
        </h2>
        <p className="text-[clamp(8px,3vw,18px)] font-[lora] font-bold md:text-base sm:text-base">
          {name}
        </p>
        <p className="text-[clamp(6px,3vw,16px)] font-[lora] font-bold md:text-sm">
          {rollNo}
        </p>
      </div>
    </div>
